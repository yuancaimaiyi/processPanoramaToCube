...
//  std::cout << intrinsic->getParams().at(0)<< " "<<
//             intrinsic->getParams().at(1) << " "<<intrinsic->getParams().at(2)<<" "
//            <<intrinsic->getParams().at(3);

  switch(current_type) 
  {
    case PINHOLE_CAMERA: 
      //OpenMVG's PINHOLE_CAMERA corresponds to Colmap's PINHOLE
      //Parameters: fx,fy, cx, cy
      std::cout <<"instrinsic : fx,fy, cx, cy \n";
      {
        ...
        std::cout << pinhole_intrinsic->K()<<"\n";
        came_line_ss << camera_id << " " << 
          "PINHOLE" << " " <<
          pinhole_intrinsic->w() << " " << 
          pinhole_intrinsic->h() << " " <<
          pinhole_intrinsic->getParams().at(0) <<" "<<
          pinhole_intrinsic->getParams().at(1) << " " <<
          pinhole_intrinsic->getParams().at(2) << " " <<
          pinhole_intrinsic->getParams().at(3) << "\n";
      }
      break;
                      ...
                      const int& floating_point_precision_digit)
{
  ...
  */ 
    ...
    iter != sfm_data.GetIntrinsics().end(); ++iter, ++my_progress_bar)
  {
    ...
    std::string camera_line;
    if (CreateLineCameraFile(camera_id + 1, intrinsic, camera_line, floating_point_precision_digit))
    {
